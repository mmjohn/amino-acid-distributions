...
# lambda value across sites - use one lambda values (0.65)
# ggplot(
#   site_cv_lambda, 
...
fit_ridge <- function(site_df){
  ...
  glmnet(
    ...
    lambda = 0.65 # optimal across sites
  ) -> fit
...
# ISSUE: the ridge regression gives rise to sites with negative counts
# if these negative counts are used in the chi squared analysis, we get
# a negative chi squared statistic and a p value of 1
# special function for ridge values
# any site that has negative counts is normalized by adding the 
# absolute value of the most negative count
# also added small constant (0.000001) because a 0 frequency for one aa 
# messes up entropy calculation

ordered_count %>% 
  group_by(site) %>% 
  mutate(
    site_min = min(est_dist_3_rid),
    site_max = max(est_dist_3_rid),
    est_dist_3_rid_adj = case_when(
      site_min < 0 & site_max > 0 ~ est_dist_3_rid+abs(site_min)+0.000001,
      TRUE ~ est_dist_3_rid
    ),
    est_rel_3_rid = est_dist_3_rid_adj/sum(est_dist_3_rid_adj),
    est_count_3_rid = sum(count)*est_rel_3_rid
  ) -> ordered_count
...
compare_eff_aa %>% 
  ...
  geom_point() +
  geom_smooth() +
  geom_hline(aes(yintercept = 0.65), linetype = "dashed") +
  scale_x_continuous(
    limits = c(1, 12.5),
...
rsq(compare_p3_rid$eff_aa, compare_p3_rid$eff_aa_est) # 0.8818093 - RIDGE
...
factor(
  regression_performance$parameters,
  levels = rev(levels(regression_performance$parameters))
) -> regression_performance$parameters
...
```{r, message=FALSE}
## code to look at actual fits 
## use ln_count and est_dist_* to see what is actually fit
## use count and est_count_* to see the real+estimated distribution that chi2 is
## performed on

# ordered_count %>%
#   filter(site == 16) %>%
#   select(site, k, est_dist_3, est_dist_3_rid,
#          est_count_3, est_count_3_rid) -> df_sub1
# 
# observed_aa_mod %>%
#   filter(site == 16) %>%
#   left_join(., df_sub1) -> df_plot
# 
# ggplot(df_plot, aes(x = k)) +
#   geom_bar(aes(y = count), fill = "grey", stat='identity') +
#   geom_point(aes(y = est_count_3, color = "violetred")) +
...
# ggplot(df_plot, aes(x = k)) +
#   geom_bar(aes(y = count), fill = "grey", stat='identity') +
#   geom_point(aes(y = est_count_3_rid, color = "violetred")) +
#   geom_line(aes(y = est_count_3_rid, color = "violetred"), size = 1) +
#   scale_color_manual(
#     name = "distribution",
#     values = c("grey" = "grey", "violetred" = "violetred"),
#     labels = c("actual", "estimated")

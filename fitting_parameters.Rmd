...
#   geom_path(aes(y = est_dist_3_rid - max(est_dist_3_rid)), color = "pink")
#   #stat_function(fun = fun.norm, color = "purple") +
...
# make correction to est_dist_3_rid because it is currently wrong
# needs to be < 0 like all other distributions, but it is not for some reason
ordered_count %>% 
  group_by(site) %>% 
  mutate(
    est_dist_3_rid_corrected = est_dist_3_rid - max(est_dist_3_rid)
  ) -> ordered_count
...
rescale_est_dist <- function(protein_df) {
  ...
  protein_df %>% 
    ...
    mutate(
      ...
      est_rel_3_rid = est_dist_3_rid_corrected/sum(est_dist_3_rid_corrected)
    ) %>% 
    mutate(

...
ordered_count %>% 
  ...
  mutate(freq_1 = (est_count_1)/sum(est_count_1),
         ...
         freq_3 = (est_count_3)/sum(est_count_3)) %>% View()
...
rsq(compare_p1$eff_aa, compare_p1$eff_aa_est) # 0.9706805

compare_eff_aa %>% 
  filter(parameters == 2 & !is.na(eff_aa_est)) -> compare_p2

rsq(compare_p2$eff_aa, compare_p2$eff_aa_est) # 0.8589123

compare_eff_aa %>% 
  filter(parameters == 3 & !is.na(eff_aa_est)) -> compare_p3

rsq(compare_p3$eff_aa, compare_p3$eff_aa_est) # 0.718942
...
dat_text <- data.frame( label = c("*R*<sup>2</sup> = 0.971", 
                                  "*R*<sup>2</sup> = 0.859", 
                                  "*R*<sup>2</sup> = 0.719"),
                        parameters = c(1, 2, 3),

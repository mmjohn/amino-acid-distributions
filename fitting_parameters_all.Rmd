...
This is the RMarkdown document used to produce Figure 7. It compares results for fitting distributions with a different number of parameters for all empirical MSAs.
...
full_join(sub1, sub2, by = c("protein", "site", "parameters")) -> eff_aa_est

left_join(eff_aa_all, eff_aa_est, by = c("protein", "site")) -> compare_eff_aa
...
full_join(results_p1, results_p2) %>% 
  ...
  pivot_longer(c(r2_1, r2_2, r2_3),
               names_to = "parameters",
               values_to = "r2") %>% 
  extract(parameters, c("trash", "param"), "(r2_)(.*)") %>% 
...
ggplot(results_corr, aes(x = param, y = r2)) +
  ...
  theme(
    ...
    axis.title.y = element_markdown()) -> fig7

fig7

save_plot("figure7.png", fig7, ncol = 1, nrow = 1, base_height = 5,

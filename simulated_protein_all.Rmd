...
Effective number of amino acids - updated; fix eff_aa_est
```{r, message=FALSE}
...
ordered_count %>%
  nest(data = c(site, aa, count, k, rel_count, ln_count,
                slope, est_dist, est_rel, est_count)) %>% 
  mutate(aa_freq = map(data, add_site_aa_freq)) %>% 
  select(-data) %>% 
  unnest(cols = aa_freq) -> site_aa_freq_est

site_aa_freq_est %>%
  mutate(flnf = frequency*log(frequency)) -> site_aa_freq_est

# # NOT WORKING - FIX FOR OTHER FIG
# site_aa_freq_est %>% 
#   nest(data = c(site, aa, count, k, rel_count, ln_count, slope, 
#                 est_dist, est_rel, est_count, frequency, flnf)) %>% 
#   mutate(eff_aa = map(data, add_eff_aa)) %>% 
#   select(-data) %>% 
#   unnest(cols = eff_aa) -> eff_aa_est
# 
...
ggplot(gam_v_effaa,
  ...
  labs(
    ...
    y = "Fitting paramter &lambda;<sup>-1</sup>",
    color = "Result"
  ) +
...
fig3
save_plot("figure3.pdf", fig3, ncol = 1, nrow = 1, base_height = 5,
